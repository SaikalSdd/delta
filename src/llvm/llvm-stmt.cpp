...
void LLVMGenerator::codegenReturnStmt(const ReturnStmt& stmt) {
...
void LLVMGenerator::codegenVarStmt(const VarStmt& stmt) {
...
void LLVMGenerator::codegenBlock(llvm::ArrayRef<Stmt*> stmts, llvm::BasicBlock* continuation) {
    beginScope();
...
void LLVMGenerator::codegenIfStmt(const IfStmt& ifStmt) {
...
void LLVMGenerator::codegenSwitchStmt(const SwitchStmt& switchStmt) {
...
void LLVMGenerator::codegenForStmt(const ForStmt& forStmt) {
...
void LLVMGenerator::codegenBreakStmt(const BreakStmt&) {
    ASSERT(!breakTargets.empty());
    builder.CreateBr(breakTargets.back());
}

void LLVMGenerator::codegenContinueStmt(const ContinueStmt&) {
    ASSERT(!continueTargets.empty());
    builder.CreateBr(continueTargets.back());
}

void LLVMGenerator::codegenCompoundStmt(const CompoundStmt& compoundStmt) {
    beginScope();
...
void LLVMGenerator::codegenStmt(const Stmt& stmt) {
    switch (stmt.getKind()) {
