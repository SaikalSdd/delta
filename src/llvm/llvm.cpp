...
void LLVMGenScope::onScopeEnd() {
    for (const Expr* expr : llvm::reverse(deferredExprs)) {
        llvmGenerator->codegenExpr(*expr);
    }

    for (auto& p : llvm::reverse(destructorsToCall)) {
        if (p.decl && p.decl->hasBeenMoved()) continue;
        llvmGenerator->createDestructorCall(p.function, p.value);
    }
}

void LLVMGenScope::clear() {
    deferredExprs.clear();
...
void LLVMGenerator::setLocalValue(llvm::Value* value, const VariableDecl* decl) {
...
llvm::Value* LLVMGenerator::getValueOrNull(const Decl* decl) {
...
llvm::Value* LLVMGenerator::getValue(const Decl* decl) {
...
llvm::Value* LLVMGenerator::getThis(llvm::Type* targetType) {
...
llvm::Type* LLVMGenerator::getEnumType(const EnumDecl& enumDecl) {
...
llvm::Type* LLVMGenerator::getStructType(Type type) {
...
llvm::Type* LLVMGenerator::getLLVMType(Type type, SourceLocation location) {
    switch (type.getKind()) {
...
void LLVMGenerator::beginScope() {
    scopes.push_back(LLVMGenScope(*this));
}

void LLVMGenerator::endScope() {
    scopes.back().onScopeEnd();
    scopes.pop_back();
}

void LLVMGenerator::deferEvaluationOf(const Expr& expr) {
...
DestructorDecl* LLVMGenerator::getDefaultDestructor(const TypeDecl& typeDecl) {
...
void LLVMGenerator::deferDestructorCall(llvm::Value* receiver, const VariableDecl* decl) {
...
void LLVMGenerator::codegenDeferredExprsAndDestructorCallsForReturn() {
    for (auto& scope : llvm::reverse(scopes)) {
        scope.onScopeEnd();
    }
    scopes.back().clear();
...
llvm::AllocaInst* LLVMGenerator::createEntryBlockAlloca(llvm::Type* type, llvm::Value* arraySize, const llvm::Twine& name) {
...
llvm::AllocaInst* LLVMGenerator::createTempAlloca(llvm::Value* value, const llvm::Twine& name) {
    auto* alloca = createEntryBlockAlloca(value->getType(), nullptr, name);
    createStore(value, alloca);
    return alloca;
}

llvm::Value* LLVMGenerator::createLoad(llvm::Value* value) {
    return builder.CreateLoad(value, value->getName() + ".load");
}

void LLVMGenerator::createStore(llvm::Value* value, llvm::Value* pointer) {
    ASSERT(pointer->getType()->isPointerTy());
    ASSERT(pointer->getType()->getPointerElementType() == value->getType());
    builder.CreateStore(value, pointer);
}

llvm::Value* LLVMGenerator::codegenAssignmentLHS(const Expr& lhs) {
...
void LLVMGenerator::createDestructorCall(llvm::Function* destructor, llvm::Value* receiver) {
...
llvm::Value* LLVMGenerator::getFunctionForCall(const CallExpr& call) {
