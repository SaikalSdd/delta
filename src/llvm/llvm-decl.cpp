...
llvm::Function* LLVMGenerator::getFunctionProto(const FunctionDecl& decl) {
...
void LLVMGenerator::codegenFunctionBody(const FunctionDecl& decl, llvm::Function& function) {
...
std::vector<llvm::Type*> LLVMGenerator::getFieldTypes(const TypeDecl& decl) {
    return map(decl.getFields(), [&](auto& field) { return getLLVMType(field.getType(), field.getLocation()); });
}

llvm::StructType* LLVMGenerator::codegenTypeDecl(const TypeDecl& d) {
...
llvm::Value* LLVMGenerator::codegenVarDecl(const VarDecl& decl) {
